...
use Drupal\parrot\Theme\OmegaSettingsInfo;
use Drupal\parrot\Export\OmegaExport;

function parrot_theme_generate_validate(&$form, &$form_state) {
  //drupal_set_message(t('Running <strong>parrot_theme_generate_validate()</strong>'));
...
function parrot_theme_generate_submit(&$form, &$form_state) {
  //drupal_set_message(t('Running <strong>parrot_theme_generate_submit()</strong>'));
  $build_info = $form_state->getBuildInfo();
  $themeHandler = \Drupal::service('theme_handler');
  // Get the theme name.
  $theme = $build_info['args'][0];
