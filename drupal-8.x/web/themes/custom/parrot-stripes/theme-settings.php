...
use Drupal\parrot-stripes\Theme\OmegaSettingsInfo;

require_once 'parrot-stripes-functions.php';
require_once 'parrot-stripes-functions--admin.php';
 ...
 */
function parrot-stripes_form_system_theme_settings_alter(&$form, \Drupal\Core\Form\FormStateInterface $form_state) 
{
    ...
    if (!$force_theme_export) {
        ...
        if ($inherit_layout) {
            ...
            $layoutProvider = parrot-stripes_find_layout_provider($theme);
        ...
        if (!$inherit_layout) {
            ...
            $layouts = parrot-stripes_return_layouts($theme);
        ...
        $form['#validate'][] = 'parrot-stripes_theme_settings_validate';
        $form['#submit'][] = 'parrot-stripes_theme_settings_submit';
        ...
        $form['actions']['submit_layout']['#submit'][] = 'parrot-stripes_theme_layout_build_submit';
        ...
        $form['actions']['generate_subtheme']['#validate'] = array('parrot-stripes_theme_generate_validate');
        $form['actions']['generate_subtheme']['#submit'] = array('parrot-stripes_theme_generate_submit');
 ...
 * @return bool
 */
function parrot-stripes_theme_exists($machine_name) 
{
    //dsm($machine_name);
    drupal_set_message(t('function <strong>parrot-stripes_theme_exists</strong> called...'));
...
function parrot-stripes_theme_settings_validate(&$form, \Drupal\Core\Form\FormStateInterface $form_state) 
{
    //drupal_set_message(t('Running <strong>parrot-stripes_theme_settings_validate()</strong>'));
}
 ...
 */
function parrot-stripes_theme_settings_submit(&$form, \Drupal\Core\Form\FormStateInterface $form_state) 
{
    // get the build info for the form
    $build_info = $form_state->getBuildInfo();
...
function parrot-stripes_theme_layout_build_validate(&$form, \Drupal\Core\Form\FormStateInterface $form_state) 
{
    //drupal_set_message(t('Running <strong>parrot-stripes_theme_layout_build_validate()</strong>'));
    $build_info = $form_state->getBuildInfo();
    // Get the theme name.
    $theme = $build_info['args'][0];
    // get all the values of the submitted form
 ...
 */
function parrot-stripes_theme_layout_build_submit(&$form, \Drupal\Core\Form\FormStateInterface $form_state) 
{
    // get the build info for the form
    $build_info = $form_state->getBuildInfo();
    // Get the theme name.
